#'
#' title: "SQL for GLAd & EDGI REMINDERS PROCESS emails "
#' "\\wsl.localhost\Ubuntu\home\k2585744\projects_wsl\R\DATABASE_Queries\..R"
#' author: "Mark Cunningham"
#' created date: "10/02/2026"
#' output: csv_file

# N: Install required packages if not already installed
{
...
# N : DONE Above / Prompt user for input
{
...
query_participants_sample <- paste( query_participants, "LIMIT 15" )
query_communications_sample <- paste( query_communications, "LIMIT 100" )
...
#' THESE next 2 statements COULD Take a CONSIDERABLE time to run
...
# 436,631 on 10 Feb 2026

# Print the dataframe
# print(df1)

# Close the connection UNLESS you need get more data
dbDisconnect(con) # DONE 10/02/2026
...
fnWriteExportImportReminderFiles <- function( ps_reminder, pdf_to_write ){
  ...
  ( dir_write <- file.path(dir_root, dir_edgi, dir_tail) )
    ...
    (fileWrite <- file.path(dir_write,
                            paste("OneOff_REMINDER_REG1_export_", as.Date(Sys.time()), sep = "")))
    ...
    # IMPORT File
    (fileWrite <- file.path(dir_write,
                            paste("OneOff_REMINDER_REG1_IMPORT_", as.Date(Sys.time()), sep = "")))
    (WriteTxtFile <- paste(fileWrite, "csv", sep = "."))
    write_csv( df_EDGI_ppts %>%
                 select(participant_id = id) %>%
                  mutate(study_id = 2,
...
# Registration 1 - Make a Difference
{
  ...
  #' For ONE OFF Reminder to be sent to ALL those who REgistered BEFORE 01/7/2025
  #' LM created List for those with registration date from 01/07/2025 until 06/01/2026
  #' see Teams conversation with her Tues 3 Feb 14.39PM
  df_1_reg1_ppts <- df_1_participants %>%
    ...
    # exclude those registered AFTER July 2025
    #' NB MUST Get Dates CORRECT here for SUBSEQUENT file generation
    #'
    filter ( dt_registered_at  < "2024-07-01" ) %>%
  ...
  #' AFTER ONE OFF Check that NONE of these PPTs overlap with LM's
  #' see Script REMINDERS_reconcile_LM_ppts.R
  #'
