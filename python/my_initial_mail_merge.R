...
#' some test simple email options
library(blastula)
library(help = "blastula")

# Compose the HTML email
email_msg <- compose_email(
  body = md("Hello, this is a test email with HTML formatting.")
)

# Add a plain-text fallback
email_msg <- add_text_body(
  email = email_msg,
  text = "Hello, this is a test email in plain text."
)
...
smtp_send(
  ...
  subject = "Test Email with Attachment and Plain Text",
  credentials = creds(
    host = "email-smtp.eu-west-1.amazonaws.com",
    port = 587,
    user = "SMTP_USERNAME",
