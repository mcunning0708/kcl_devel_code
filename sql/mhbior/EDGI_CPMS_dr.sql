...
ps.id, 
ns.legacy_code,
COALESCE(sub.min_created_at, ps2.consented_at) as consented_at, 
...
SELECT cf.label, cf.study_id, cfr.participant_id, MIN(cfr.created_at) AS min_created_at
    ...
    -- WHERE cfr.participant_id = 10476 
...
SELECT cf.label, cfr.participant_id, MIN(cfr.created_at) AS min_created_at
    FROM consent_form_responses cfr
    INNER JOIN consent_forms cf 
	on ( cf.id = cfr.consent_form_id ) 
    WHERE cfr.participant_id = 10476 and SUBSTR(cf.label, 1,4) = 'EDGI' 
    GROUP BY label, participant_id;
...
SELECT 
    ps.id, ps2.study_id, ps2.export_eligibility_state, ps2.manual_eligibility_state 
-- cf.label as consent_form
FROM participants ps 
where ps.id = 10476 
INNER JOIN participant_study ps2 ON ps.id = ps2.participant_id; 
