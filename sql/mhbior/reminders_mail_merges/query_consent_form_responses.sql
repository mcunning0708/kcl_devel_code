...
-- if you want find using a GLAD / EDGI format ID 
SET @alias := /*prompt*/ 'GLAD000214';
select @alias;
select CONCAT('%', @alias, '%' );
select * from participants where aliases like CONCAT('%', @alias, '%' ) ;
-- or using ID 88407 ? 20 ? 
...
# SHOW ALL ALIASES for THIS ppt ( @participant_id )
SELECT 
	ps.id, jt.alias_matched Aliases
    FROM participants ps 
		CROSS JOIN JSON_TABLE( ps.aliases, '$[*]' COLUMNS( alias_matched VARCHAR(255) PATH '$'  ) ) AS jt
    WHERE ID = @participant_id; 

# SHOW ONLY the GLAD / EDGI ALIASES for THIS ppt ( @participant_id )
SELECT 
	ps.id, jt.alias_matched LEGACY_STUDY_ID
    FROM participants ps 
		CROSS JOIN JSON_TABLE( ps.aliases, '$[*]' COLUMNS( alias_matched VARCHAR(255) PATH '$'  ) ) AS jt
    WHERE 
		TRIM(jt.alias_matched) REGEXP '^(EDGI|GLAD)[0-9]{6}$' AND ID = @participant_id; 
    
show columns from participant_study;
select count(*) from participant_study;
...
-- CONSENT FORM ANALYSIS / QUERIES ...
show columns from consent_forms;
select * from consent_forms;
select count(*) from consent_forms;
-- INFORMATION SHEETS
show columns from information_sheets;
select * from information_sheets;
select count(*) from information_sheets;
select * from participant_study where participant_id = @participant_id;
...
SET @consent_form_id := /*prompt*/ '5';
show columns from consent_form_items;
select count(*) from consent_form_items; -- 167 items
select count(*) from consent_form_items WHERE consent_form_id = @consent_form_id;
select * from consent_form_items WHERE consent_form_id = @consent_form_id;

select * from consent_form_items cfi WHERE (cfi.content) LIKE '%newsletter%'; 

show columns from consent_form_item_responses;
select * from consent_form_item_responses where participant_id = @participant_id;
